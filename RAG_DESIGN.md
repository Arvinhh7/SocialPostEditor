...
`roles` 保存名称、描述和 `identity_rules`。所有 `posts`、`profiles`、`feedback` 和 `generation_runs` 必须带 `role_id`。在正式多用户产品中，查询条件还必须包含 `tenant_id`，并由服务端从登录态注入，不能接受客户端自由指定租户。
...
反馈保存“AI 初稿—本人终稿—修改原因”。生成时只检索最相关的少量反馈，而不是把全部反馈塞入提示词。建议把原因写成可复用规则，例如“删除绝对化结论，改成有条件判断”，不要只写“不像我”。
